        ...
        case D_ACEDIT:
        {
            if(response)
            {
                switch(listitem)
                {
                    ...
                    case 4:
                    {
                        new query[328], string[328], id = ActorCreate[playerid][ae_IDActor];
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(PRESSED(KEY_NO)) // Key 'N'
    {
        ...
        {
            format(string, sizeof string, "%s mówi: Moje ubranie to %i i mam id %i", ActorCache[actorid][aName], ActorCache[actorid][aSkin], ActorCache[actorid][aID]);
...
stock LoadAllCreateActor()
    return mysql_tquery(SQL_ID, "SELECT * FROM actor", "OnLoadAllCreateActor");
